  ...
  if (loading) {
    return (
      <section className="mt-5 px-4">
        <div className="bg-white rounded-[12px] p-5 shadow-sm border border-brand-teal/10 animate-pulse">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-14 bg-gray-100 rounded-[8px]" />
            ))}
          </div>
        </div>
      </section>
  ...
  return (
    <section className="mt-5 px-4">
      ...
      {draws.slice(0, 3).map((draw) => (
        <Link key={draw.key} href={`/lottery/${draw.key}/chart`} className="bg-white rounded-[12px] p-4 mb-2.5 shadow-sm border border-brand-teal/10 block active:scale-[0.98] transition-transform">
