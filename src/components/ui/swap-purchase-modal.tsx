...
export default function SwapPurchaseModal({ product, onClose, onSuccess }: SwapPurchaseModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-black/60 flex items-end sm:items-center sm:p-4" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-sm sm:mx-auto rounded-t-[28px] sm:rounded-[8px] p-5 pb-8" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="text-center mb-4">
          ...
          <div className="text-xl font-bold text-brand-gold-dark mt-1">¥{product.price}</div>
        ...
        {/* Payment Mode */}
        <div className="bg-amber-50 rounded-[8px] p-3 mb-4">
          ...
          <div className="flex gap-2">
            {[
              ...
              <button key={m.key} onClick={() => { setPayMode(m.key); setIdleBeanAmount(Math.min(price, price * Number(product.max_idle_bean_ratio))); }}
                className={`flex-1 py-2 rounded-[8px] text-xs transition-all ${payMode === m.key ? "bg-brand-gold text-white shadow-sm" : "bg-white text-text-secondary border border-amber-200"}`}>
        ...
        {/* Summary */}
        <div className="bg-bg rounded-[8px] p-3 text-xs text-text-secondary mb-4 space-y-1">
        ...
        <button onClick={handleBuy} disabled={buying}
          className="w-full py-3 bg-gradient-to-r from-brand-gold to-brand-gold-dark text-white font-semibold rounded-[8px] text-sm disabled:opacity-50 shadow-lg shadow-brand-gold/25 active:scale-[0.98] transition-transform">
          {buying ? "购买中..." : "确认购买"}
        </button>
