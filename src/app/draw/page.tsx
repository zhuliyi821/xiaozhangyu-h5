    ...
    <main className="min-h-screen bg-bg pb-24">
        ...
        <div className="flex gap-1 px-3 pb-2 overflow-x-auto scrollbar-none">
          {Object.entries(LOTTERY_CONFIG).map(([key, conf]) => (
            <button key={key} onClick={() => { setActiveKey(key); setPage(1); }}
              className={`flex-shrink-0 px-3.5 py-1.5 rounded-[12px] text-xs font-medium transition-colors ${
                activeKey === key
      ...
      {loading ? (
        <div className="p-4 space-y-3">
          {[1,2,3].map(i => <div key={i} className="h-24 bg-surface rounded-[20px] animate-pulse" />)}
        </div>
      ) : (
