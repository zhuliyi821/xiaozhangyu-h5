...
function BirthInfoForm({ birthDate, setBirthDate, birthHour, setBirthHour, onSubmit, onClose }: {
  ...
  return (
    <div className="bg-white rounded-[20px] p-5 shadow-sm border border-gray-100 text-left">
      ...
      <div className="space-y-3">
        <div>
          ...
          <input type="date" value={birthDate} onChange={e => setBirthDate(e.target.value)}
            className="w-full px-3 py-2 border border-gray-200 rounded-[12px] text-xs outline-none focus:border-brand-teal focus:ring-2 focus:ring-brand-teal/20" />
        </div>
        <div>
        ...
        <button onClick={onSubmit}
          className="w-full py-2.5 bg-gradient-to-r from-brand-teal to-brand-teal-dark text-white rounded-[14px] text-xs font-semibold active:scale-[0.97] transition-transform shadow-sm">
          生成我的运势
        </button>
      </div>
    </div>
            ...
            <>
              ...
              <div className="space-y-2.5">
                <button onClick={() => setShowLogin(true)}
                  className="w-full py-3 bg-gradient-to-r from-brand-teal to-brand-teal-dark text-white rounded-[14px] text-sm font-semibold active:scale-[0.97] transition-transform shadow-sm">
                  登录查看运势
                </button>
                <button onClick={() => { setGuestMode(true); setLoading(true); setRetryKey(k => k + 1); }}
                  className="w-full py-2.5 border border-gray-200 text-text-secondary rounded-[14px] text-xs font-medium active:scale-[0.97] transition-transform">
                  游客体验（无个性化）
                </button>
              </div>
            ...
            <>
              ...
              <div className="flex gap-2 justify-center">
                <button onClick={() => { setError(""); setLoading(true); setRetryKey(k => k + 1); }}
                  className="px-5 py-2 bg-gradient-to-r from-brand-teal to-brand-teal-dark text-white rounded-[12px] text-xs font-medium active:scale-[0.97] transition-transform">
                  重试
                </button>
                <button onClick={() => { setGuestMode(true); setLoading(true); setRetryKey(k => k + 1); }}
                  className="px-5 py-2 border border-gray-200 text-text-secondary rounded-[12px] text-xs font-medium active:scale-[0.97] transition-transform">
                  游客模式
                </button>
              </div>
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-brand-teal-light/20 via-white to-white pb-28">
      ...
      <div className="px-4 pt-5">
        ...
        {/* ═══════ Hero 区 ═══════ */}
        <div className="bg-gradient-to-br from-brand-teal via-brand-teal-dark to-brand-teal-dark rounded-[24px] p-6 mb-5 text-white shadow-lg shadow-brand-teal/30">
        ...
        <div className="bg-white rounded-[20px] p-5 shadow-sm border border-gray-100 mb-5">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-shrink-0">
              ...
              <div className="bg-brand-teal-light/10 rounded-[12px] p-2.5 border border-brand-teal-light/30">
            ...
            {hexagram.changed_hexagram && (
              <div className="flex-shrink-0">
                ...
                <div className="bg-gray-50 rounded-[12px] p-2.5 border border-gray-100">
                  {hexagram.yao && [...hexagram.yao].reverse().map((y, i) => {
        ...
        <div className="bg-white rounded-[20px] p-5 shadow-sm border border-gray-100 mb-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-brand-teal-light/15 rounded-[14px] p-3">
        ...
        {/* ═══════ 当前时辰 ═══════ */}
        {nowHour && (
          <div className="bg-gradient-to-r from-brand-teal to-brand-teal-dark rounded-[20px] p-5 shadow-sm mb-5 text-white">
          ...
          <div className="overflow-x-auto scrollbar-none -mx-1">
            <div className="flex gap-2 min-w-max px-1">
              {hourly.map((h, i) => {
                ...
                return (
                  <div key={i} className={`flex flex-col items-center gap-1 p-2 rounded-[12px] min-w-[52px] transition-all ${isNow ? "bg-brand-teal-light/30 ring-2 ring-brand-teal ring-offset-1" : "bg-brand-teal-light/10"}`}>
        ...
        {/* 免责声明 */}
        <div className="p-3.5 rounded-[16px] bg-brand-teal-light/10 border border-gray-100 text-[9px] text-gray-400/60 text-center leading-relaxed">
