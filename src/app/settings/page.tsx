    ...
    <main className="min-h-screen bg-bg pb-24">
      ...
      {loading && (
        <div className="px-4 mt-6 space-y-3">
          {[1, 2, 3].map(i => <div key={i} className="h-12 bg-surface rounded-[8px] animate-pulse" />)}
        </div>
      )}
      ...
      {profile && (
        <div className="px-4 mt-6 space-y-4">
          ...
          {/* 提示消息 */}
          {msg && (
            <div className={`text-center text-xs py-2 rounded-[8px] ${msg.startsWith("✅") ? "bg-green-50 text-green-600" : "bg-red-50 text-red-500"}`}>
              {msg}
            </div>
          )}
        </div>
