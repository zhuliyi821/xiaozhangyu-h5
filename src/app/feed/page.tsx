...
export default function FeedPage() {
  ...
  return (
    <main className="min-h-screen bg-bg pb-24">
      ...
      {/* Post Input */}
      <div className="mx-4 mt-3 bg-surface rounded-[16px] p-3 border border-[rgba(69,204,213,0.08)]">
        <textarea value={postContent} onChange={e => setPostContent(e.target.value)}
          placeholder={user ? "分享你的预测心得..." : "登录后可以发动态"}
          rows={2} maxLength={2000}
          className="w-full bg-bg rounded-[12px] px-3 py-2 text-xs resize-none outline-none focus:ring-2 focus:ring-brand-teal/30" />
      ...
      {loading ? (
        <div className="mx-4 mt-3 space-y-3">
          {[1,2,3].map(i => <div key={i} className="h-28 bg-surface rounded-[16px] animate-pulse" />)}
        </div>
      ) : feeds.length === 0 ? (
        <div className="mx-4 mt-8 p-8 text-center bg-surface rounded-[20px]">
        ...
        <div className="mx-4 mt-3 space-y-3">
          {feeds.map(feed => (
            <div key={feed.id} className="bg-surface rounded-[16px] p-4 border border-[rgba(69,204,213,0.08)]">
              {/* Header */}
