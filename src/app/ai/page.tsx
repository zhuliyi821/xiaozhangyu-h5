...
export default function AIChatPage() {
  ...
  const doSend = async (msg: string, cost: number) => {
    ...
    try {
      const r = await fetch(`${API_BASE}/api/v2/ai/chat`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_id: user?.uid || 0,
              ...
              <div className="flex flex-wrap gap-2">
                {cfg.questions.map((q, i) => (
                  <button key={i} onClick={() => handleQuickQuestion(q, cfg.cost)}
                    className="text-[10px] bg-brand-teal/10 text-brand-teal-dark rounded-[8px] px-3 py-1.5 active:scale-95 transition-transform hover:bg-brand-teal/20">
                    {q}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {cfg.cost_map?.slice(0, 4).map((item: {label: string; cost: number}) => (
                  <button key={item.label} onClick={() => { setSubCategory(item.label); setZodiacCost(item.cost); }}
                    className="text-[10px] bg-brand-gold-light/30 text-brand-gold-dark rounded-[8px] px-3 py-1.5 active:scale-95 transition-transform">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center px-6">
          <div className="bg-white rounded-[8px] p-5 max-w-sm w-full shadow-xl">
            ...
            <button onClick={confirmDisclaimer} disabled={!disclaimerAccepted}
              className={`w-full rounded-[8px] py-2.5 text-sm font-semibold text-white transition ${
          ...
          <div className="bg-white rounded-[8px] p-5 max-w-sm w-full shadow-xl">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowConfirm(false)}
                className="flex-1 rounded-[8px] py-2.5 text-xs font-medium border border-border-tertiary text-text-secondary active:scale-95 transition-transform">取消</button>
              <button onClick={confirmSend} disabled={balance < deductCost}
                className={`flex-1 rounded-[8px] py-2.5 text-xs font-semibold text-white active:scale-95 transition-transform ${
