  ...
  return (
    <main className="pb-24 min-h-screen bg-bg">
      ...
      <div className="px-4 pt-4">
        {/* Guide Banner */}
        {showGuide && (
          <div className="bg-gradient-to-r from-brand-teal/10 to-brand-teal/5 rounded-[8px] border border-brand-teal/20 p-3 mb-3 flex items-start gap-3">
        ...
        {/* Tabs */}
        <div className="flex gap-1 rounded-[8px] bg-bg p-0.5 mb-4">
          {[
            ...
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`relative flex-1 rounded-[8px] px-3 py-2 text-center text-xs font-medium transition ${
        ...
        {tab === "trade" && (
          <div className="space-y-4">
            {/* Price Chart */}
            <div className="bg-surface rounded-[8px] p-3 shadow-sm border border-border-tertiary">
            ...
            <div className="bg-surface rounded-[8px] p-4 shadow-sm border border-border-tertiary">
              ...
              <div className="mb-4">
                ...
                <div className="mt-1.5 flex gap-2">
                  <button onClick={() => setDirection(1)}
                    className={`flex-1 rounded-[8px] py-2.5 text-xs font-medium transition ${
              ...
              <div className="mb-4">
                ...
                <input type="number" min="10" value={points} onChange={e => setPoints(e.target.value)}
                  className="w-full rounded-[8px] border border-border-tertiary bg-bg p-2.5 text-sm outline-none focus:border-brand-teal" placeholder="自定义金额" />
              </div>

              <button onClick={openPosition} disabled={loading}
                className={`w-full rounded-[8px] py-3 text-sm font-semibold text-white transition active:scale-[0.97] ${
          ...
          <div className="space-y-2">
            {positions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <button onClick={() => setTab("trade")} className="mt-3 rounded-[8px] bg-gradient-to-r from-brand-teal to-brand-teal-dark px-5 py-2 text-xs font-medium text-white shadow-sm active:scale-95 transition-transform">
                  去开仓
                </button>
              </div>
            ) : (
              positions.map(p => (
                <div key={p.id} className="bg-surface rounded-[8px] p-3 shadow-sm border border-border-tertiary">
                  ...
                  <button onClick={() => closePosition(p.id)} disabled={loading}
                    className="mt-2 w-full rounded-[8px] border border-border-tertiary py-1.5 text-[10px] text-text-secondary transition hover:bg-bg active:scale-[0.98]">
                    {loading ? "⏳" : "平仓"}
                  </button>
                </div>
              ))
        ...
        {tab === "history" && (
          <div className="space-y-2">
            {trades.length === 0 ? (
              ...
              trades.map(t => (
                <div key={t.id} className="bg-surface rounded-[8px] p-3 shadow-sm border border-border-tertiary">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
