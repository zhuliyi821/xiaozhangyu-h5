 ...
 *
 * 路由规则：
 *   store-{id}.surplus.hi.cn/*   →  /store/{id}/*
 *   {tenant}.surplus.hi.cn/*      →  /tenant/{tenant}/*
 *   h5.surplus.hi.cn/*            →  (默认 H5 路由)
 *   merchant.surplus.hi.cn/*      →  /merchant/*
 */
...
export function middleware(request: NextRequest) {
  ...
  // hostname = "store-10001.surplus.hi.cn" → subdomain = "store-10001"
  // hostname = "h5.surplus.hi.cn" → subdomain = "h5"
  ...
  // 处理通配子域名路由
  if (subdomain) {
    // 门店专属 H5: store-{id}.surplus.hi.cn
